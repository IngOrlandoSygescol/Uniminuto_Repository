...
function Tipo2({
  ...
  const changeCheck = (e: any) => {
    ...
    if (selected.length != 3 && selected.length < 3) {
      ...
      setValues({ ...values, correcta: value });
    } else {
      alert("No puede seleccionar mas de dos opciones de respuesta");
      e.preventDefault();
      return false;
    }
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!values?.correcta) {
      alert("Debe seleccionar las respuestas correctas");
      return false;
    }
    if (values?.correcta?.split("@").length < 1) {
      alert(
        "Debe seleccionar dos respuestas correctas, en caso contrario utilice el tipo de Pregunta 1"
      );
      return false;
    }
    if (
