...
export type Props = {
  competencia: string;
  prueba: number;
  preguntasIngresadas: any;
  setShowModal: React.Dispatch<React.SetStateAction<VisibilidadModal>>;
};
export function Preguntas({ competencia, prueba, setShowModal, preguntasIngresadas }: Props) {
        ...
        <div className="container mx-auto w-11/12 md:w-2/3 max-w-4xl  ">
          <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400 ">
            ...
            <div className="flex gap-2 flex-wrap justify-around">
              {console.log('data origigal : ', data)}
              {console.log('preguntasIngresadas: ', preguntasIngresadas)}
              {preguntasIngresadas?.map((preg: any, key: number) => {
                return (
                  <>
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        setShowQuestion({ ...preg });
                      }}
                      title={`Pregunta ${(preg?.aprobo == 0 && "Pendiente") ||
                        (preg.aprobo == 1 && "Rechazada") ||
                        (preg.aprobo == 2 && "Aprobada")
                        }`}
                      className={`${(preg?.aprobo == 0 &&
