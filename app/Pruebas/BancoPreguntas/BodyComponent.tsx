...
const BodyComponent = () => {
  ...
  const GetPreguntasCoa = async (Coa: string, Tipo: string) => {
    try {
      ...
      const data = await fetch( `/api/Pruebas/BancoPreguntas/GetBancoPreguntas?Coa=${Coa}&Tipo=${Tipo}`).then((res) => res.json());
      console.log(data);
  ...
  const TipoPrueba = [ { value: "SP", label: "Pruebas de Ingreso" }, { value: "SS", label: "Pruebas de Egreso" },];
  ...
  const getData = async () => {
    try {
      ...
      const data: any = await fetch(`/api/Pruebas/BancoPreguntas/GetCoa`).then(  (res) => res?.json() );
