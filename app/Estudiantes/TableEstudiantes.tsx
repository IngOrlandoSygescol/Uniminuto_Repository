...
type Props = {
  ...
  setShowImage: React.Dispatch<React.SetStateAction<{ Image: string; Visible: boolean }>>;
  InfoUser: { IdRol: number };
};
      ...
      selector: (row: any) => (
        <div className=" block  lg:flex lg:gap-3 px-6 py-4 font-normal text-gray-900">
          <div
            ...
            onClick={() => {
              setShowImage({
                Image: `https://sygescol.uniminuto.sistemasivhorsnet.com/sygescol2023/images/fotos/administrativos/${row?.Imagen || "SinFotoAdmin.png"
                  }`,
                Visible: true,
              });
            }}
          >
            <Image
              src={`https://sygescol.uniminuto.sistemasivhorsnet.com/sygescol2023/images/fotos/administrativos/${row?.Imagen || "SinFotoAdmin.png"
                }`}
              alt="Foto Estudiante"
              width={40}
              height={40}
...

    {
      ...
      selector: (row: any) => (
        <div className="flex flex-wrap justify-center items-center text-red-900">
          {localStorage.getItem('usu_rol') != '2' && (
            <>
              {" "}
              <button
  ...
  const subHeaderComponentMemo = useMemo(() => {
    return (
      <input
        ...
        // onClear={handleClear}
        value={filterText}
        placeholder="Buscar por nombres y apellidos o número de documento"
