...
type props = { InfoUser: { IdRol: number } };

const BodyComponent = ({ InfoUser }: props) => {
  const [ShowModal, setShowModal] = useState({ AddVisible: false, EdditVisible: false } as VisibilidadModal);
  const [Estudiante, setEstudiante] = useState<Estudiante[]>([]);
  const [InfoEditar, setInfoEditar] = useState({} as Estudiante);
  const [ShowImage, setShowImage] = useState<{ Image: string; Visible: boolean; }>({ Image: "", Visible: false });
  ...
  const getData = async () => {
    try {
      ...
      const data: any = await fetch(`/api/Configuracion/CargaMasiva/GetStudents?SubSede=${SubSede}`).then((res) => res.json());
