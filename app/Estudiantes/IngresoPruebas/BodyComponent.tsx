              ...
              <form>
                <div className="mb-2">
                  ...
                  <Select
                    ...
                    getOptionLabel={(item: any) => `Prueba de ingreso #${item.IdPrueba}`}
                    ...
                    <button
                      onClick={async (e) => {
                        try {
                          ...
                          Swal.fire({
                            imageUrl: '/EscudoUniminuto.png',
                            title: 'Apreciado estudiante...',
                            text: RegistrarAsistencia?.body,
                            customClass: { image: 'img-swal' }
                          }).then(() => {
                            Swal.fire({
                              imageUrl: '/EscudoUniminuto.png',
                              title: 'Recuerde...',
                              text: 'Cada competencia tiene asignado un tiempo específico para el desarrollo de la prueba, tenga en cuenta, que una vez finalizado ese tiempo, el sistema saltará automáticamente a la competencia siguiente, sin permitir su retorno.',
                              customClass: { image: 'img-swal' }
                            })
                          });
                          setValues({} as any);
                          setData((prev) => prev.filter((item) => item.IdPrueba !== Values?.PruebaID));
                        } catch (error) {
                          console.log(error);
                          Swal.fire({
                            icon: 'error',
