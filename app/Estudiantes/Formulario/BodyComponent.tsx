...
const BodyComponent = ({ docu, InfoUrl }: Props) => {
  ...
  const [data, setData] = useState<any>({ info: {}, save: [] });
  ...
  const SectionFormated = sectionForm.map((info) => {
    const compara = data?.save?.find((e: any) => e.Seccion === info.comparar);
    return { ...info, save: !!compara };
  });
  ...
  useEffect(() => {
    const GetData = async () => {
      try {
        setCargando(true);
        const res = await fetch(`/api/Estudiantes/QueremosConocerte/BaseInfoColegio?SubSede=${localStorage.getItem('IdSubSede')}`);
        const data = await res.json();
        setData(data.DatosUniversidad);
        setCargando(false);
      } catch (error) {
      ...
      <div className="container max-w-6xl mx-auto bg-blue-50">
        ...
        {Cargando ? (
          ...
          <div className="" >
            ...
            <section className="text-center text-xl text-blue-600 " >
              <b>
