...
function ModalVictimaConflicto({ setVisible, Documento }: Props) {
  ...
  const handerSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const sentDataRes = await fetch(
      `/api/Formulario/Save/BaseInfoVictima?num=${Documento}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ InputSelect }),
      }
    ).then((res) => res.json());
  ...
  const getData = async () => {
    try {
      const InfoBase = await fetch(`/api/Formulario/Save/BaseInfoVictima?num=${Documento}`).then((res) => res.json());
      setData(InfoBase);
    } catch (error) {
      console.error(error);
