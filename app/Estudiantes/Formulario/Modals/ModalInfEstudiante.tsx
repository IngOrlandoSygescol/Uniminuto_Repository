...
const ModalInfEstudiante = ({ setVisible, Documento }: Props) => {
  ...
  const getData = async () => {
    ...
    try {
      const InfoBase = await fetch(`/api/Estudiantes/QueremosConocerte/InformacionEstudiante?num=${Documento}`).then((res) => res.json());
      setData(InfoBase);
  ...
  const handerSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (sentDataRes.msg == "Datos actualizados correctamente") { setVisible({ DatosAcademicos: false }) }
  };
                  ...
                  <div className="grid gap-4 mb-6 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                      ...
                      <Select
                        ...
                        getOptionLabel={(e: any) => { return e.nombre }}
                        options={data.GrupoSanguineo}
                        onChange={(e: any) => {
                          setInputData({ ...InputData, Rh: e });
                        }}
                      placeholder={InputData?.Rh[0]?.nombre || "Seleccione uno"}
                      />
                    </div>
                    <div>
                      <label
                    ...
                    <div>
                      ...
                      <input
                        autoComplete="off"
                        className="w-full p-[0.5rem] bg-gray-200 mt-2  rounded border-2 border-slate-200 text-gray-700 "
                        type="text"
                  ...
                  <div className="grid gap-4 mb-6 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                      ...
                      <Select
                        className="text-gray-900"
                        isDisabled={!sisben && true}
                        getOptionLabel={(e: any) => { return e.municipio_nombre }}
                        options={munSisb}
                        onChange={(e: any) =>
                          setInputData({
                            ...InputData,
