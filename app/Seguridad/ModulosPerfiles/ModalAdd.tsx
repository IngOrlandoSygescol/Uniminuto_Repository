...
const ModalAdd = ({ setShowModal, setPerfiles }: Props) => {
  ...
  const hanlerSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const sentDataRes = await fetch("/api/Seguridad/AddPerfil", {
      method: "POST",
      headers: {
  ...
  const GetData = async () => {
    const getModulos = await fetch("/api/Seguridad/GetModulosSelect").then(
      (res) => res.json()
    );
              ...
              <>
                <div className=" max-h-[23rem] overflow-auto ">
                  <h2 className="font-bold text-3xl text-blue-700 text-center">
                    Lista de módulos - Rol {Tipo[Values?.Rol - 1]?.label}
                  </h2>
