...
const DatosInput = () => {
  ...
  const disableInputs = () => {
    if (localStorage) {
      const usu_rol = localStorage.getItem('usu_rol');
      if (usu_rol === '3' || usu_rol === '2') {
        setTimeout(() => {
  ...
  useEffect(() => {
    const fetcher = async () => {
      ...
      if (SubSede != 0 || SubSede != undefined) {
        ...
        setInputValues({ ...InfoBase?.InfoBase, } as any);
        setData(InfoBase);
      }
      setIsPending(false);
    };
      ...
      {isPending ? (
        ...
        <div>
            ...
            {Data && (
              <>
                <div className="text-sm sm:text-lg  flex flex-col items-center justify-center lg:text-xl font-bold">
                  <h1 className=" uppercase">{Data?.InfoBase?.NombreSubSede || ""}.</h1>
                  <h3 className="">{`${Data?.InfoBase?.direccion}`}</h3>
                  <h3 className="">{`Telefonos: ${Data?.InfoBase?.telefono1} - ${Data?.InfoBase?.telefono2}`}</h3>
                  <h3 className="">Nit: {`${Data?.InfoBase?.nit}`}</h3>
                </div>
              </>
            )}
          </section>
          ...
          <form onSubmit={HanlerSubmit}>
            ...
            {
              localStorage.getItem('usu_rol') !== '2' && localStorage.getItem('usu_rol') !== '3' ? (
                <div className="flex">
