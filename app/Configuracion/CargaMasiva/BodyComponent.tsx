...
const BodyComponent = () => {
  ...
  const [Data2, setData2] = React.useState({ IdSubSede: localStorage.getItem("IdSubSede") || "" } as any);
  ...
  const getData = async () => {
    try {
      ...
      const res: any = await fetch(`/api/Configuracion/CargaMasiva/GetStudents?SubSede=${SubSede}`).then((res) => res.json());
            ...
            onClick={async (e) => {
              ...
              try {
                ...
                for (const iterator of result) {
                  const res = await axios.post(`/api/Configuracion/CargaMasiva/AddEstudiantes?SubSede=${Data2?.IdSubSede}`, { Estudiantes: iterator, });
                  console.log(res);
