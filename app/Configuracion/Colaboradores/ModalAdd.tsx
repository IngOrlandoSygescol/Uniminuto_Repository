...
const ModalAdd = ({ setShowModal, setAdministrativos }: Props) => {
  ...
  const handerSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const validationEmail = EmailValidator?.validate(Values?.Correo || '');
    ...
    if (Values?.IdSubSede === "0" && Object.keys(Values).length !== 10) {
      ...
      return;
    } else if (Object.keys(Values).length !== 9) {
      alert("Por favor, llene todos los campos");
      return;
    }
    ...
    try {
      ...
      const ColaboradoresRes = await fetch(
        `/api/Configuracion/Administrativos/GetAdministrativos?SubSede=${searchParams.get("SubSede") || 0
        }`
      ).then((res) => res.json());
              ...
              <div className="grid sm:grid-cols-1 gap-2 ">
                <div className="mb-2">
                  <label className="mb-3 block text-base font-medium text-gray-800">
                    Selecciona CU <span className="text-red-900">(*)</span>
                  </label>
                  ...
                  <Select
                    ...
                    onChange={(item: any) => {   setValues({ ...Values, IdSubSede: item.Id })}}
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              ...
              <div className="mb-2">
                ...
                <Select
                  ...
                  getOptionLabel={(item: any) => item.Nombre}
                  getOptionValue={(item: any) => item.Id}
                  onChange={(item: any) => { setValues({ ...Values, Perfil: item.Id }); }}
            ...
            <div className="grid ">
              <div className="mb-2 ">
                ...
                <input
                  ...
                  id="Usuario"
                  disabled
                  value={`${Values?.Nombre?.split(" ")[0]?.toLowerCase() || ""
