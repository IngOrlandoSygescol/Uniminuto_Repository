...
const BodyComponent = () => {
  const [Values, setValues] = useState({ IdSubSede: localStorage.getItem("IdSubSede") || "", } as any);
  ...
  const TipoPrueba = [
    { value: "SP", label: "Pruebas de Ingreso" },
    { value: "SS", label: "Pruebas de Egreso" },
  ];
  ...
  const GetProgramas = async (subSede: any) => {
    const ProgramasRes = await axios(`/api/Configuracion/Programas/GetProgramas?SubSede=${subSede}`);
    console.log('res ', ProgramasRes);
  ...
  useEffect(() => {
    if (Values?.Programa) GetSemestres(Values?.Programa)
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const info = await fetch(`/api/Configuracion/GradosGrupos/GetInfoModal?SubSede=${SubSede}`).then((res) => res?.json());
      setData({ ...Data, Programas: info?.programa || [] });
      setIsPending(false);
    };
