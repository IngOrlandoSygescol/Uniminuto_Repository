...
const Estudiantes = ({
  ...
  const [selectedDateRange, setSelectedDateRange] = useState<any>({
    startDate: startDateEstudiantes ? new Date(startDateEstudiantes) : new Date(),
    endDate: endDateEstudiantes ? new Date(endDateEstudiantes) : new Date(),
    key: 'selection',
  });

  
  const fechaFinAprobacion = new Date(EndDateAprobacion);
    ...
    <div>
      <section className="flex justify-center">
        <div>
          ...
          >
            Rango fechas para presentar la prueba{" "}
            <span className="text-red-900">(*)</span>
          </label>
          <DateRange
            ranges={[selectedDateRange]}
            minDate={fechaFinAprobacion}
            rangeColors={["#3b82f6"]}
