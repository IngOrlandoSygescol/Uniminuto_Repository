...
type props = {
  ...
  setInfoEditar: React.Dispatch<React.SetStateAction<Competencia | {}>>;
  setShowModal: React.Dispatch<React.SetStateAction<boolean>>;
};
const TableCompetencias = ({
  info,
  setCompetencias,
  setInfoEditar,
  setShowModal,
  ...
  const columns = [
    ...
    {
      name: "Abreviatura",
      selector: (row: any) => row?.Abreviatura,
      sortable: true,
      wrap: true,
    },
    {
      name: "Ponderación",
      selector: (row: any) => row?.Ponderacion,
      sortable: true,
      wrap: true,
    },
    {
      ...
      selector: (row: any) =>
        row?.TipoCompetencia == "G" ? "Genérica" : "Específica",
      sortable: true,
      wrap: true,
    },
    {
            ...
            <button
              ...
              onClick={async () => {
                const validate = confirm(
                  "¿Está seguro de eliminar esta Competencia?"
                );
                if (validate) {
                  // fecha de eliminación
                  try {
                    const responseRemove = await fetch(
                      "/api/Configuracion/Competencias/DeleteCompetencia",
                      {
                        method: "DELETE",
                        headers: {
  ...
  return (
    <div className="border-t-2 border-t-white">
      ...
      <DataTable
        ...
        responsive
        noDataComponent="No hay Competencias registradas"
...
export default TableCompetencias;
