...
type props = {
  ...
  setInfoEditar: React.Dispatch<React.SetStateAction<Programa>>;
...
const TableProgramas = ({
  ...
  if (localStorage.getItem('usu_rol') !== '2') {
    columns.push({
      name: "Operaciones",
      selector: (row: any) => (
        <>
                ...
                if (validate) {
                  ...
                  try {
                    const responseRemove = await fetch(
                      "/api/Configuracion/Programas/DeletePrograma",
                      {
                        method: "DELETE",
                        headers: {
  ...
  return (
    <div className="border-t-2 border-t-white">
      <DataTable
        ...
        responsive
        noDataComponent="No hay Programas registrados"
