...
const ModalAdd = ({ setShowModal, setProgramas, InfoEditar }: Props) => {
  ...
  const [cu, setCu] = useState<{ value: string; label: string }[]>([]);
            ...
            <div className="">
              <div className="mb-2">
                <label
                  ...
                  className="mb-3 block text-base font-medium text-gray-800"
                >
                  A qué CU se va a asociar? <span className="text-red-900">(*)</span>
                </label>
                <Select
                  required
                  options={cu}
                  onChange={(item: any) => { setValues({ ...Values, Cu: item.value }) }}
                  placeholder="Seleccione un Opción"
                  value={cu.find(option => option.value === Values.Cu)}
                  className="dark:text-black"
                />
              </div>
            </div>
