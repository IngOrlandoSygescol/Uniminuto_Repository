...
const AddSubSede = ({ Config, setShowModal, setSubSedes }: Props) => {
  ...
  const handerSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      const ResAdd = await axios.post("/api/Configuracion/SubSedes/AddSubSede", { ...Values, id });
      const SubSedeRes = await axios.get("/api/Configuracion/SubSedes/GetSedesSubSedes");
      ...
      <div className="container mx-auto  w-11/12 md:w-2/3 max-w-2xl">
        <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
          <h1 className="text-center text-lg tracking-normal leading-tight mb-4 bg-[#151A8B] w-full text-white p-4 rounded-lg font-bold ">
            Agregar <span title="Centro Universitario">(CU)</span>
          </h1>
            ...
            <>
              <div className="">
                <div className="mb-2">
                  ...
                  >
                    Nombre{" "}
                    <span title="Centro de operaciones académico">(CU)</span>{" "}
                  ...
                  <input
                    ...
                    placeholder="Ingrese el nombre del Centro Universitario"
