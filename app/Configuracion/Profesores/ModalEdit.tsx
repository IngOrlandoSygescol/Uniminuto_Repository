...
const ModalUpgrade = ({ setShowModal, InfoEditar, setDocentes }: Props) => {
  ...
  const [cu, setCu] = useState<Option[]>([]);

  const selectedOption = cu.find(option => option.value === Values.Cu);

  const getGetAllSedesSubSedes = async () => {
    const GetProgramasRes = await fetch(`/api/Configuracion/SubSedes/GetAllSedesSubSedes`)
    const data = await GetProgramasRes.json()
            ...
            <div className="max-h-[30rem] overflow-auto scrollbar-hide rounded-lg bg-gray-50 p-2  ">
              <div className="bg-white rounded-lg shadow-xl ">
                ...
                <div className="flex flex-col items-center -mt-[10.3rem]">
                  ...
                  <Image
                    src={`https://sygescol.uniminuto.sistemasivhorsnet.com/sygescol2023/images/fotos/administrativos/${InfoEditar?.Imagen || "SinFotoAdmin.png"
                      }`}
                    width={160}
                    height={160}
                    alt="Foto de perfil"
                        ...
                        <div className="mb-2">
                          ...
                          <MultiSelect
                            ...
                            onChange={(selectedOptions: any) => {
                              let cuSelect: any = [];
                              selectedOptions.forEach((cu: any) => cuSelect.push(cu.label));
                              const cuJoin = cuSelect.join(',');
                              setValues({ ...Values, Cu: cuJoin });
                        ...
                        <div className="mb-2">
                          ...
                          {Data.documentos?.length && (
                            <Select
                              ...
                              onChange={(item: any) => { setValues({ ...Values, TipoDocumento: item.id }) }}
                      ...
                      <div className="mt-6 grid sm:grid-cols-2 gap-2">
                        <div className="mb-2">
                          ...
                          <input
                            ...
                            id="Usuario"
                            disabled
                            value={`${Values?.Nombre?.split(" ")[0] || ""}${Values?.Apellidos?.split(" ")[0] || ""
                              }`}
                            onChange={hanlerChange}
                            className="InputStyle"
                          />
