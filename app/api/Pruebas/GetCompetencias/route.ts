...
export async function POST(req: NextRequest) {
  try {
    ...
    if (Values?.TipoPrueba == "SP" && Values?.IdRol == "2") {
      ...
      SELECT
        asignacionPrueba.id as Id,
        pfc_ejes.eje_tip as Tipo,
        pfc_ejes.eje_nom as Nombre,
        asignacionPrueba.competencia AS idCompetencia,
        asignacionPrueba.Hora,
        asignacionPrueba.Minutos
      FROM
        asignacionPrueba
      INNER JOIN
        parametros_pruebas ON (asignacionPrueba.prueba = parametros_pruebas.id)
      INNER JOIN
        pfc_ejes on (asignacionPrueba.competencia = pfc_ejes.eje_id)
      WHERE
        parametros_pruebas.id = '${Values?.PruebasId}'
    ...
    `);
      ...
      for (const competencia of competenciasRes) {
        ...
        const [preguntas]: any = await connectionPool.query(
          `SELECT * FROM preguntas_pruebas WHERE
