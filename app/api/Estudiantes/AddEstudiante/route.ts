...
export async function POST(req: NextRequest) {
  try {
    ...
    const [ExistenciaAlumno]: any = await connectionPool.query(`SELECT alumno_num_docu FROM pfc_alumno WHERE alumno_num_docu = '${NumeroDocumento}'
    `);
   ...
   SELECT MAX(alumno_id) AS MaxIdAlumno FROM pfc_alumno`);
    ...
    //   console.log(`
    //  INSERT INTO pfc_alumno (alumno_rum, tipo_docu_id, alumno_num_docu, alumno_ape1, alumno_ape2, alumno_nom1,alumno_nom2, alumno_celular, alumno_email) values  ('${MaxIdAlumno[0].MaxIdAlumno + 1
    //     }', '${TipoDocumento}', '${NumeroDocumento}', '${Apellidos?.split(
    //       " "
    //     )[0]?.toUpperCase()}', '${Apellidos?.split(" ")[1]?.toUpperCase() || ""
    //     }', '${Nombre?.split(" ")[0]?.toUpperCase()}', '${Nombre?.split(" ")[1]?.toUpperCase() || ""
    //     }', '${Telefono}', '${Email}')`);
 ...
 INSERT INTO usuario (login, pass, rol, tipo, idUsuario,subsede) values  ('${NumeroDocumento}','${NumeroDocumento}','3','ESTUDIANTE','${InsertAlumno?.insertId}','${IdSubSede}')`);
    ...
    if (!InsertUsuario?.insertId) return NextResponse.json({ body: "Error al ingresar el usuario" }, { status: 400 });

    return NextResponse.json({ body: "Estudiante Agregado con éxito" }, { status: 200 });
