...
export async function GET(req: NextRequest) {
  ...
  try {
    if (IdRol == "3") {
      ...
      const [InfoMatriculaEstudiante]: any = await connectionPool.query(`SELECT programa,semestre,matri_id FROM pfc_matricula WHERE alumno_id='${IdUser}' and subSedeId='${SubSede}'`);
      ...
      if (InfoMatriculaEstudiante?.length == 1) {
        ...
        const [PruebasResult]: any = await connectionPool.query(
          ...
          WHERE
            parametros_pruebas.id NOT IN (
              SELECT
                respuestas_estudiante.prueba
              FROM
                respuestas_estudiante
              WHERE
                respuestas_estudiante.estudiante = '${matri_id}'
            )
      ...
      //  AND parametros_pruebas.programa = '${programa}'
        ...
        if (PruebasResult.length == 0) return NextResponse.json({ pruebas: [] }, { status: 200 })
