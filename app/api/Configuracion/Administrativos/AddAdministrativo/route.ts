...
export async function POST(req: NextRequest) {
  try {
    ...
    const [VerificarExistencia]: any = await connectionPool.query(
      `SELECT * FROM usuario WHERE login = '${Nombre?.split(" ")[0]
        ?.normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "") || ""
      }${Apellidos?.split(" ")[0]
        ?.normalize("NFD")
    ...
    const [AdministrativoAdd]: any = await connectionPool.query(`
      INSERT INTO admco (nombre, admco_ape1, admco_ape2, admco_nom1, admco_nom2, documento, tipo_documento, cargo,mail,genero,subSedeId) values ('${Apellidos} ${Nombre}', '${Apellidos?.split(" ")[0]?.toUpperCase() || ""
    ...
    const [AddUser]: any = await connectionPool.query(`
      INSERT INTO usuario (login, pass, rol, tipo, idUsuario,subsede) values ('${Nombre?.split(" ")[0]
        ?.toLowerCase()
        ?.normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "") || ""
      }${Apellidos?.split(" ")[0]
        ...
        ?.normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "") || ""
      }', '${NumeroDocumento}', '${Perfil || ""}', 'ADMINISTRATIVO', '${AdministrativoAdd.insertId
      }','${IdSubSede}')`);
