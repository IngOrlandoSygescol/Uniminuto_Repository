...
export async function POST(req: NextRequest) {
  try {
    ...
    if (Object.keys(InputValues || {}).length === 0) {
      return NextResponse.json({ body: "Datos vacios" }, { status: 400 });
    }

    const [VerificarExistencia]: any = await connectionPool.query(`SELECT id FROM datosUniversidad WHERE id = '${InputValues.id}'`);
