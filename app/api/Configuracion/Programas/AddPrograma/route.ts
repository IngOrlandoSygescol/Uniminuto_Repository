...
export async function POST(req: NextRequest) {
  try {
    const { Nombre, Sigla, IdSubSede, Periodicidad, Cu } = await req?.json();
    ...
    if (Nombre && Sigla) {
      ...
      const [addPrograma]: any = await connectionPool.query(`
          INSERT INTO pfc_programa (pro_nom, pro_sigla,subSedeId,periodicidad,cu) VALUES ('${Nombre}', '${Sigla}','${IdSubSede}','${Periodicidad}','${Cu}')
        `);
