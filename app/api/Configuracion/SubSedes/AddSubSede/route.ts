...
export async function POST(req: NextRequest) {
  try {
    ...
    const [SedeExist]: any = await connectionPool.query(`SELECT id FROM subSedes WHERE nombre = '${NombreCoa}'`);

    if (SedeExist.length > 0) {
      return NextResponse.json({ body: "La sede ya existe" }, { status: 200 });
    }
