...
export async function GET(req: any) {
  ...
  try {
    ...
    let consulta = `SELECT 
      rol, rol_nombre, idUsuario, tipo, subsede,ChangePass 
      FROM usuario 
      INNER JOIN rol 
      ON usuario.rol = rol.rol_id 
      WHERE login = '${Usuario?.toLowerCase()}' 
      AND pass = '${Pass}'
    `;
    ...
    if (user?.length > 0) {
      ...
      if (user[0]?.tipo == "ADMINISTRATIVO") {
        const [Administrativo]: any = await connectionPool.query(
          `
          SELECT 
          concat(admco.admco_nom1,' ',admco.admco_nom2) as Nombre,
          concat(admco.admco_ape1,' ',admco.admco_ape2) as Apellidos,
          admco.documento as Documento,
          admco.cargo as Cargo 
          FROM admco 
          WHERE id = ${user[0]?.idUsuario}
        `
        );
        ...
        DemasInfo = {
          ...
          Cargo: Administrativo[0]?.Cargo,
          RolTipo: user[0]?.rol,
          Id: user[0]?.idUsuario,
        };
        console.log(DemasInfo);
