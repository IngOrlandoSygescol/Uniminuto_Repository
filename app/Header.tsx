...
const Header = ({
  ...
  const [perfil, setPerfil] = useState<Modulo[]>([]);
  ...
  const [nameSubSede, setNameSubSede] = useState("");
  const router = useRouter();

  const GetInfoBase = () => {
    if (localStorage?.usu_rol) {
      setSession({
        usu_rol: localStorage?.usu_rol,
        rol_nombre: localStorage?.rol_nombre,
        menu: JSON.parse(localStorage?.menu),
        DemasInfo: JSON.parse(localStorage?.DemasInfo),
        IdSubSede: localStorage?.IdSubSede,
        nameSubSede: localStorage?.nameSubSede,
      });

    }

    const rol = localStorage.getItem("usu_rol");

    if (rol == "2") {
      setPerfil(navDocente);
    } else if (rol == "3") {
      setPerfil(navEstudiante);
    } else if (rol == "4" || rol == "11" || rol == "13" || rol == "14") {
      setPerfil(navCoordinador);
    } else if (rol == "5") {
      setPerfil(navAdmin);
    } else if (rol == "6" || rol == "7") {
      setPerfil(navRector);
    } else if (rol == "1") {
      setPerfil(navSoporte);
    }
  };

  const getNameSede = async () => {
    const data = await fetch(
      `/api/Configuracion/SubSedes/getSubSedeId?id=${localStorage?.IdSubSede}`
    );
    const res = await data.json();
    if (res.body == "La sede no existe") {
      return;
    } else {
      setNameSubSede(res);
    }
  ...
  function capitalizarPalabras(cadena: any) {
  ...
  return (
    <>
      <div className="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased bg-white dark:bg-gray-700 text-black dark:text-white">
        <div className="border-r-2 fixed flex flex-col top-0 left-0 w-12 hover:w-64 md:w-64 bg-[#070e54] dark:bg-gray-900 h-full text-white transition-all duration-300 z-10 sidebar ">
          <div className="border-r-2  flex items-center justify-between md:justify-center w-12 md:w-64 h-25 bg-[#070e54] dark:bg-gray-800">
            <div className="flex justify-center">
              <span className="w-13 my-2 h-8 md:w-16 md:h-16 mr-2 rounded-md cursor-pointer overflow-hidden">
                <Link href="/">
                  <Image
            ...
            <div className="text-center p-2 header-data-personal" style={{ fontSize: "14px" }}>
              <p className="mb-2">PERFIL : {
                Session?.DemasInfo?.Cargo ? `${Session?.DemasInfo?.Cargo || ""}`
                  : Session?.rol_nombre
              }
              </p>

              <p className="mb-2">CU : {nameSubSede}</p>

              <p className="hidden md:block md:break-normal mb-2">
                {Session?.DemasInfo?.Nombre ? `${Session?.DemasInfo?.Nombre || ""}
                ${Session?.DemasInfo?.Apellidos || ""}` : Session?.rol_nombre}
              </p>
            </div>
          </div>
          ...
          <div className="scrollbar-hide overflow-x-hidden flex flex-col justify-between flex-grow ">
            <ul className="flex flex-col py-4 space-y-1">
                  ...
                  <ul>
                    {item?.SubModulos?.map((subItem: any) => (
                      <li className="cursor-pointer" key={subItem.id}>
                        <Link
                          href={`${subItem?.Link}?SubSede=${Session?.IdSubSede
                            }&IdRol=${Session?.usu_rol}&IdUser=${Session?.DemasInfo?.Id
                        ...
                        >
                          <div
                            className={`relative flex flex-row items-center h-11 focus:outline-none hover:bg-blue-800 text-white-600 hover:text-white-800  hover:border-blue-500  ${active == subItem.id &&
                          ...
                          >
                            ...
                            <span className="ml-2 text-sm leading-[1] ">
                              {capitalizarPalabras(subItem?.SubModulo)}
                            </span>
                          </div>
                        </Link>
                      </li>
              ...
              <li>
                <Link href="/MapaRectorias">
                  <div className="cursor-pointer flex flex-row items-center h-11 focus:outline-none hover:bg-blue-800 dark:hover:bg-gray-600 text-white-600 hover:text-white-800  border-transparent hover:border-blue-500  pr-6">
                    <span className="inline-flex justify-center items-center ml-3 sm:ml-4">
                      <svg
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth={1.5}
              ...
              <li>
                <div
                  ...
                  className=" cursor-pointer flex flex-row items-center h-11 focus:outline-none hover:bg-blue-800 dark:hover:bg-gray-600 text-white-600 hover:text-white-800 border-transparent hover:border-blue-500  pr-6"
                >
                  <span className="inline-flex justify-center items-center ml-3 sm:ml-4">
                    <svg
                      className="w-6 h-6"
            ...
            <p className="mb-14 px-5 py-3 hidden md:block text-center  text-xs">
              ...
              <a
                href="https://www.sistemasivhorsnet.com/"
                target="_blank"
                rel="noopener noreferrer"
              >
                Copyright @Ivhorsnet S.A.S {new Date().getFullYear()} -{" "}
                {new Date().getFullYear() + 1}
              </a>
            </p>
          </div>
        </div>
        <div className="h-full flex flex-col md:ml-64">{children}</div>
      </div>

      {ModalCambiosPass && (
        <ModalCambio
          setModalCambiosPass={setModalCambiosPass}
          IdUpdate={Session?.DemasInfo?.Id}
        />
      )}
